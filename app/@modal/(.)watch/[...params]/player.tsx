...
export default function ZXCPlayer({
  ...
  closePlayer: boolean;
  setClosePlayer: (closePlayer: boolean) => void;
}) {
  ...
  const videoRef = useVideoSetup(localData);
  const {
    progress,
    handleSliderChange,
    togglePlay,
  ...
  useEffect(() => {
    if (englishSubId && selectedSub === "" && localData) {
  ...
  return (
    ...
    >
      ...
      {!isLoading && (
        ...
        >
          <div className="absolute top-0 lg:p-10 p-4 flex justify-between inset-x-0">
            <ArrowLeft
              size={30}
